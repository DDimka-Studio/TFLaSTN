 ...
 * `-text <string>`: The string or numerical sequence you wish to process.
 * `-file <path>`: Read the input content from the specified file.
 * `-out <path>`: Save the processing result to the specified file instead of the console.
...
**2. Encoding a file and saving the result:**
```bash
./TFLaSTN -file input.txt -e --gap -out encoded.txt
```
*(Reads input.txt, encodes it, and saves the result to encoded.txt)*

**3. Encoding text into 6-byte HEX (Strict Mode):**
```bash
./TFLaSTN -text "Test" -e --hex-format
```
*Output: Formatted HEX: 20:54:73:74:00:00*

**4. Decoding a numeric sequence from a file:**
```bash
./TFLaSTN -file encoded.txt -d --gap -out decoded.txt
```
*(Reads encoded.txt, decodes it, and saves the original text to decoded.txt)*
 ...
 * **error:404 - Invalid HEX input string:** Triggered if the input provided via `--hex-format` is malformed.
 ...
 * **Error: Invalid or conflicting arguments:** Triggered if required flags are missing, contradictory, or if no input source is provided.
